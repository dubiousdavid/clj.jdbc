...
(defn make-prepared-statement
  ...
  ([conn sqlvec {:keys [lazy? result-type result-concurency fetch-size max-rows holdability]
                 :or {fetch-size 0 result-type :forward-only result-concurency :read-only}
                 :as options}]
